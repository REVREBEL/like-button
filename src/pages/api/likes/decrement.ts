...
import { atomicDecrement } from '../../../lib/likes-views/kv-atomic';
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    // Atomically decrement likes (never below 0)
    const newLikeCount = await atomicDecrement(KV, likesKey, 1);
    ...
    return errorResponse('Failed to decrement likes', 500);
  }
};
